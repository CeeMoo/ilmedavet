...
switch ($yap) {
	case 'ekle':
		...
		if(!empty($hata)){
			...
			$ekle=$db->insert('yazili_dersler')->set(array(
				'baslik' => $baslik,
				'slug' => $slug,
				'uye_id' => session('id'),
				'aciklama' => $aciklama,
			...
			if($ekle){
				...
				go(url_admin.'?sayfa=yazili_dersler', 'js', 2);
			}else{
	...
	case 'duzenle':
		...
		$bak=$db->from('yazili_dersler_kat')->where('id', $kat_id)->first();
		...
		if(!empty($hata)){
			...
			$slug=sef_link($baslik);
			$upla=$db->update('yazili_dersler')->where('id', $id)->set(array(
				'baslik' => $baslik,
				'slug' => $slug,
				'aciklama' => $aciklama,
				'duz_id' => session('id'),
	...
	case 'yazili_dersler_kat_ekle':
		...
		if(!empty($baslik) && is_numeric($aktif)){
			$slug=sef_link($baslik);		
			$ekle=$db->insert('yazili_dersler_kat')->set(array(
				'baslik' => $baslik,
				'slug' => $slug,
				'ust_kat' => $ust_kat,
				'aktif' => $aktif
			...
			if($ekle){
				echo alert('Ekleme işlemi başarılıdır.', 3);
				go(url_admin.'?sayfa=yazili_dersler&alt=yazili_dersler_kat', 'js', 2);
			}else{
				echo alert('Başarısız işlem lütfen sistem yetkilisi ile görüşün', 2);
			}
		}else{
	...
	case 'yazili_dersler_kat_duzenle':
		$id=p('id');
		$baslik=p('baslik');
		$ust_kat=p('ust_kat');
		$aktif=p('aktif');
		...
		if(!empty($baslik) && is_numeric($aktif)){
			$slug=sef_link($baslik);
			$upla=$db->update('yazili_dersler_kat')->where('id', $id)->set(array(
				'baslik' => $baslik,
				'slug' => $slug,
				'ust_kat' => $ust_kat,
				'aktif' => $aktif
